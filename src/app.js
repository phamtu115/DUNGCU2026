...
function renderPayments() {
  ...
  const groupCards = [...groups.entries()].filter(([, invoices]) => invoices.length > 1).map(([groupId, invoices]) => {
    ...
    return `<div class="card payment-group"><div class="section-head"><div><h3>${esc(invoices[0].guestName)}</h3><p>${esc(groupId)} · ${invoices.length} phòng: ${esc(rooms)}</p></div><span class="status ${due ? 'waiting' : 'paid'}">${due ? 'Còn thanh toán' : 'Đã hoàn tất'}</span></div><div class="summary"><div><span>Tổng hóa đơn</span><b>${money(total)}</b></div><div><span>Cọc + đã thu</span><b>${money(deposit + paid)}</b></div><div><span>Phụ thu</span><b>${money(surcharge)}</b></div><div><span>Giảm tiền</span><b>${money(discount)}</b></div><div><span>Còn phải thu</span><b>${money(due)}</b></div></div>${outstanding.length > 1 ? `<div class="actions"><button class="button success" data-action="open-group-payment" data-id="${esc(groupId)}">Thanh toán gộp ${outstanding.length} phòng</button></div>` : ''}</div>`;
  }).join('');
